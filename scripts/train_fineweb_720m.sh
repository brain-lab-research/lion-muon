...
# Architecture: n_layer=12, n_embd=2048, n_head=16, seq_len=1024
# Param count: ~709M; trained at ~5 tokens/param ~= 3.54B tokens
...
# --- Algorithms ---
# LRs scaled down by 0.5 from 355M based on muP heuristic (1024/2048)

run "adamw" \
  --opt adamw --lr 2.5e-3 --beta1 0.8 --beta2 0.999

run "muon" \
  --opt sign_muon --lr 3.75e-4 --muon_lr_factor 3.75e-4 \
  --muon_every_k 1 --cheap_mode sign --momentum 0.95 --nesterov True

run "signum" \
  --opt sign_muon --lr 3.75e-4 --sign_lr 3.75e-5 \
